...
public class TimesEducationrss extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Uri uri=Uri.parse(links1.get(position));
                Intent i=new Intent(Intent.ACTION_VIEW,uri);
                startActivity(i);
            }
        });
        new TimesEducationrss.ProcessInBackground().execute();
    }
    ...
    public class ProcessInBackground extends AsyncTask<Integer,Integer, Exception> {
        ...
        ProgressDialog progressDialog=new ProgressDialog(TimesEducationrss.this);
        Exception exception;
        @Override
                ...
                while(event!=XmlPullParser.END_DOCUMENT){
                    if(event==XmlPullParser.START_TAG){
                        if(xpp.getName().equalsIgnoreCase("item")){
                            ...
                            if(insideitem){
                                titles1.add(xpp.nextText());
                            }
                        }
                        else if(xpp.getName().equalsIgnoreCase("link")){
                            if(insideitem){
                                links1.add(xpp.nextText());
                            }
                        }
                    }
        ...
        protected void onPostExecute(Exception s) {
            ...
            ArrayAdapter<String> adapt=new ArrayAdapter<String>(TimesEducationrss.this,android.R.layout.simple_expandable_list_item_1,titles1);
            lv.setAdapter(adapt);
